...
# ***** TODO ******
# 1. review parser arguments (end of this file) and set split_id before running
# 2. set one of the following as true, has to correspond with the features associated with the chosen split_id
ALL_CAT = False  # use all cat cols
F_CLASSIF = True  # use features selected using sklearn's f_classif
ALL = False  # use all features
TERM = False  # use features from COLS_TERM

# all cat cols (maybe comment out numerical preprocessing)
ALL_FEATURE_SELECTION = False  # use all selected features from the feature selection directory
if ALL_CAT:
    ...
    COLS = [LABEL_COL] + CATEGORICAL_COLS
    BINARY_COLS = []
    DROP_COLS = []

# selectKBest w/ f_classif cols
# (some features are commented out because of redundancy)
elif F_CLASSIF:
    CATEGORICAL_COLS = ["CU_MAP_SEGMENT",
    ...
    COLS = ["CU_AGE",
            ...
            "MPP_DEVICE_OS_TYPE",
            "MPP_DEVICE_PRODUCERNAME",
            # added modelnames: related to *_producername in dimension reductions
            'CU_U_MAIN_DEV_MODELNAME',
            'MPP_DEVICE_MODELNAME'] + [LABEL_COL]
    BINARY_COLS = []
    DROP_COLS = []

# all unique features from feature_selection directory
# (some features are commented out because of redundancy)
elif ALL_FEATURE_SELECTION:
    CATEGORICAL_COLS = ['CLM_LIVSFASE_SEGMENT',
                        'CU_ADSL_OK_RESULT',
                        'CU_GENDER',
                        'CU_MAP_SEGMENT',
                        'CU_U_MAIN_DEV_CATEGORY',
                        'CU_U_MAIN_DEV_OS_TYPE',
                        'CU_U_MAIN_DEV_PRODUCERNAME',
                        'MPP_BINDING_TYPE',
                        'MPP_DEVICE_CATEGORY',
    ...
    _CATEGORICAL_COLS = ["CU_GENDER", "CU_EMAIL_IND", "CU_SMS_IND",
                         "CU_MAP_SEGMENT", "CLM_LIVSFASE_SEGMENT", "CU_FYLKE",
                         "CU_HOUSE_TYPE", "HH_MOB_COVERAGE_IN", "HH_MOB_COVERAGE_OUT",
                         "CU_ADSL_OK_RESULT", "CU_VDSL_OK_RESULT", "CU_LINE_STATUS",
                         "CU_U_MAIN_BRAND", "CU_U_MAIN_SUBS_TYPE", "CU_U_MAIN_DEV_MODELNAME",
    ...
    CATEGORICAL_COLS = [i for i in _CATEGORICAL_COLS if i in COLS_TERM]
    BINARY_COLS = [i for i in _BINARY_COLS if i in COLS_TERM]
    COLS = COLS_TERM
    DROP_COLS = []

# not using X features
X_CATEGORICAL_COLS = []
X_BINARY_COLS =[]

# not using embeddings
_CAT_EMB_DIM = {'CU_U_MAIN_DEV_MODELNAME': 76,'CU_GENDER': 2, 'X_ENSLIG_PAR': 2, 'HH_MOB_COVERAGE_IN': 2,
               ...
               'CU_FYLKE': 19, 'CU_U_MAIN_DEV_TOUCH_SCREEN': 2, 'CU_ADSL_OK_RESULT': 4,
               'CU_HOUSE_TYPE': 6, 'MPP_PRODUCT_NAME': 10, 'MPP_BINDING_PRODUCT_DESC': 32,
...
def load_data():
    # removed SQL query
    if ALL:
        df = pd.read_csv("handset_data_train_wo_X.csv")
    else:
...
def get_distinct_values(df, cat_vars, x_vars=False):
    # comment out dim reductions that relate to features not in the chosen CATEGORICAL_COLS

    for var in CATEGORICAL_COLS:
        df.loc[df.loc[:,var].isnull(), var] = "UNKNOWN"

    if 'MPP_DEVICE_OS_TYPE' in CATEGORICAL_COLS:
    ...
    if 'CU_U_MAIN_DEV_PRODUCERNAME' in CATEGORICAL_COLS:
    ...
    if 'MPP_DEVICE_PRODUCERNAME' in CATEGORICAL_COLS:
    ...
    if 'CU_U_MAIN_DEV_MODELNAME' in CATEGORICAL_COLS and 'CU_U_MAIN_DEV_PRODUCERNAME' in CATEGORICAL_COLS:
    ...
    if 'MPP_DEALER_NAME' in CATEGORICAL_COLS:
    ...
    if 'MPP_DEVICE_MODELNAME' in CATEGORICAL_COLS and 'MPP_DEVICE_PRODUCERNAME' in CATEGORICAL_COLS:
...
#
# Output: A dataframe containing the encoded categorical variables.
#

# takes a really long time
# reduce categorical feature dimensions before using
...
#
#  Output: A dataframe, with dim*len(vars) columns, containing the encoded categorical variables.
#
...
def train_and_evaluate_model(model, data_train, data_test, nb_epochs=100,
    ...
    # model.save('model_%d.h5' % (args.data_split_id))
    # print("saved model")
...
def load_and_preprocess_data(args):
    ...
    # preprocess cat data with numerical values
    if 'CU_MAP_SEGMENT' in CATEGORICAL_COLS:
    ...
    if 'MPP_CHURN_SEGMENT_ADJ' in CATEGORICAL_COLS:
