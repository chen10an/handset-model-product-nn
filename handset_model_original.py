...
DROP_COLS = ["ID", "CU_U_MAIN_SUBSCRIPTION_KEY", "CU_U_MAIN_DEV_MODEL_ID", "MPP_DEVICE_MODEL_ID", "X_LIVSFASE",
             "MPP_NET_DISCOUNT_OTHER_FEE"]
LABEL_COL = "TARGET_S_TO_S_APPLE"
CATEGORICAL_COLS = ["CU_GENDER", "CU_EMAIL_IND", "CU_SMS_IND", "CU_MAP_SEGMENT",
                    "CLM_LIVSFASE_SEGMENT", "CU_FYLKE", "CU_HOUSE_TYPE", "HH_MOB_COVERAGE_IN",
                    "HH_MOB_COVERAGE_OUT", "CU_ADSL_OK_RESULT", "CU_VDSL_OK_RESULT", "CU_LINE_STATUS",
                    "CU_U_MAIN_BRAND", "CU_U_MAIN_SUBS_TYPE", "CU_U_MAIN_DEV_MODELNAME", "CU_U_MAIN_DEV_OS_TYPE",
                    "CU_U_MAIN_DEV_PRODUCERNAME", "CU_U_MAIN_DEV_CATEGORY", "CU_U_MAIN_DEV_TYPE",
                    "CU_U_MAIN_DEV_HD_VOICE",
                    "CU_U_MAIN_DEV_TOUCH_SCREEN", "CU_U_MAIN_DEV_LTE", "MPP_BRAND", "MPP_SUBS_START_REASON",
                    "MPP_CHANGETYPE", "MPP_PRODUCT_NAME", "MPP_CHURN_SEGMENT_ADJ", "MPP_DEVICE_MODELNAME",
                    "MPP_DEVICE_OS_TYPE", "MPP_DEVICE_PRODUCERNAME", "MPP_DEVICE_CATEGORY", "MPP_DEVICE_TYPE",
                    "MPP_DEVICE_HD_VOICE", "MPP_DEVICE_TOUCH_SCREEN", "MPP_DEVICE_LTE", "MPP_BINDING_PRODUCT_DESC",
                    "MPP_BINDING_TYPE", "MPP_DEALER_NAME", "MPP_SALES_CH", "MPP_DEALER_CHAIN",
                    "MPP_IN_PORT_SP_NAME"]
X_CATEGORICAL_COLS = ["X_ENSLIG_PAR", "X_POSTREKLAMESEGMENTER_LAST1", "X_BOLIGTYPE_ENKEL", "X_BOLIGTYPE_ENKEL_HIST",
                      "X_MERKE_NYESTE_BIL", "X_BILSEGMENT_NYESTE_BIL"]
BINARY_COLS = ["CU_EMAIL_ACCEPT_FLAG", "CU_SMS_AKSEPT_FLAG", "CU_RES_TELENOR_DM", "CU_RES_TELENOR_TM",
...
X_BINARY_COLS = ["X_HUSEIER", "X_HYTTEEIER"]

CAT_EMB_DIM = {'X_POSTREKLAMESEGMENTER_LAST1': 10, 'X_BILSEGMENT_NYESTE_BIL': 5, 'CU_U_MAIN_DEV_MODELNAME': 128,
               'CU_GENDER': 2, 'X_ENSLIG_PAR': 2, 'HH_MOB_COVERAGE_IN': 2,
               'CU_U_MAIN_DEV_PRODUCERNAME': 128, 'CLM_LIVSFASE_SEGMENT': 6, 'CU_EMAIL_IND': 2,
               'CU_U_MAIN_DEV_CATEGORY': 5, 'MPP_DEVICE_MODELNAME': 128,
               ...
               'MPP_DEVICE_PRODUCERNAME': 128, 'MPP_DEVICE_CATEGORY': 5, 'MPP_DEALER_NAME': 32,
               'CU_U_MAIN_DEV_TYPE': 9, 'MPP_SUBS_START_REASON': 3, 'MPP_DEVICE_LTE': 2, 'MPP_DEALER_CHAIN': 16,
               'CU_VDSL_OK_RESULT': 4, 'MPP_CHURN_SEGMENT_ADJ': 10, 'CU_U_MAIN_DEV_LTE': 2, 'MPP_DEVICE_OS_TYPE': 5,
...
SHARED_EMBEDDING = [[7, 8], [9, 10], [12, 22], [14, 27], [15, 28], [16, 29], [17, 30], [18, 31], [19, 32], [20, 33],
                    [21, 34]]
X_SHARED_EMBEDDING = []
...
def get_distinct_values(cat_vars, x_vars=True):
    fname = "cat_levels_{}_x.pickle".format("w" if x_vars else "wo")
    if os.path.isfile(fname):
        cat_vars_dict = pickle.load(open(fname, "rb"))
    else:
        QUERY = 'SELECT DISTINCT({}) from crm.{}'
        tables = ['S_TO_S_APPLE_TRAIN', 'S_TO_S_APPLE_VALID']
        cat_vars_dict = {}
        cnx = get_db_connection()
        ...
        for var in cat_vars:
            values = set()
            for t in tables:
                df = pd.read_sql_query(QUERY.format(var, t), con=cnx)
                df.fillna("missing", inplace=True)
                values |= set(df[var].str.decode('utf-8').str.lower())
            cat_vars_dict[var] = list(values)
...
def _integer_encode(df, cat_levels):
    df_cat = pd.DataFrame(df[cat_levels.keys()].copy())
    for key, values in cat_levels.iteritems():
        if df_cat[key].dtype == 'O':
            df_cat[key].fillna("missing", inplace=True)
            df_cat.loc[:, key] = df_cat.loc[:, key].str.decode('utf-8').str.lower()
        else:
            df_cat[key].fillna("missing", inplace=True)
...
def _one_hot_encode(df, cat_levels, binary_enc=False):
    ...
    for key, values in cat_levels.iteritems():
        for value in values:
            df_sp[key + "_" + str(value)] = (df[key].astype('str')).apply(lambda x: 1 if x == str(value) else neg)
...
def preprocess_data(df, cat_levels, data_to_process=['num', 'cat'], cat_encoding='hashing', cat_emb_dim=32, standardize=True,
                    pca_whiten=False, pca_reduce=None, log_transform=False, binary_enc=False, include_x_vars=True,
    ...
    if 'cat' in data_to_process:
        ...
        if cat_encoding == 'hashing':
            ...
            df_cat = _integer_encode(df, cat_levels)
...
def create_model(num_vars, cat_vars, cat_encoding='hashing', cat_emb_dim=32, cat_levels=None,
                 include_x_vars=True, activation='prelu'):
...
def train_and_evaluate_model(model, data_train, data_test, nb_epochs=100,
    ...
    earlystopping = EarlyStopping(monitor='val_loss', patience=earlystop_pat, verbose=0, mode='auto')
    ...
    if oversample:
        ...
        history = model.fit_generator(gen.generator(),
                                  validation_data=(
                                      [data_test["num"].values] +
                                      [data_test["cat"][col].values for col in data_test["cat"].columns],
        ...
        history = model.fit([data_train["num"].values] +
                            [data_train["cat"][col].values for col in data_train["cat"].columns],
...
class OverSamplingBatchGenerator:
    ...
    def generator(self):
        ...
        while True:
            ...
            for start_idx in xrange(0, self.N * self.n_batch_neg, self.n_batch_neg):
                ...
                batch = [np.vstack([self.data_train["num"].loc[batch_pos_idx].values,
                        ...
                        [np.vstack([self.data_train["cat"].loc[batch_pos_idx, col].values.reshape((self.n_batch_pos,1)),
                                    self.data_train["cat"].loc[batch_neg_idx, col].values.reshape((self.n_batch_neg,1))])
                         for col in self.data_train["cat"].columns]
                batch.append(labels)
                batch = shuffle(*batch)
...
def load_and_preprocess_data(args):
    ...
    cat_levels = get_distinct_values(cat_vars, args.x_vars)

    if not (os.path.isfile(fname_num) and os.path.isfile(fname_cat)):
...
if __name__ == "__main__":
    ...
    parser.add_argument('--epochs', default=100, type=int, help="Nr of epochs. Default is 100", dest="epochs")
    parser.add_argument('--batch_size', default=32, type=int, help="Batch size. Default is 32", dest="batch_size")
    ...
    parser.add_argument('--smooth_factor', default=0.1, type=float,
    ...
    parser.add_argument('--activation', default='prelu',
                        ...
                        dest="activation")
    parser.add_argument('--x_vars', default=True, type=bool_arg, help="If True (default), include X variables",
                        dest="x_vars")
    parser.add_argument('--std', default=True, type=bool_arg, help="If True (default), standardize data.",
                        dest="std")
    parser.add_argument('--pca_whiten', default=True, type=bool_arg, help="If True (default), PCA-whiten data.",
                        dest="pca_whiten")
    parser.add_argument('--pca_reduce', default=0.9, type=float,
                        ...
                        dest="pca_reduce")
    parser.add_argument('--cat_enc', default='integer',
                        help="Encoding to be used for categorical variables. Default is 'integer' " \
                             ...
                             "'hashing_all', 'one-hot'.",
                        dest="cat_enc")
    parser.add_argument('--log_xform', default=True, type=bool_arg, help="If True (default), log-transform data.",
                        dest="log_xform")
    ...
    parser.add_argument('--data_split_id', default=0, type=int,
